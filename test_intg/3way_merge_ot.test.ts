...
maybe('<remote/sync_worker> threeWayMerge() with OT', () => {
   ...
   */
  it.skip('resolves case 5 - Conflict. Accept theirs (create)', async () => {
    const [dbA, dbB, remoteA, remoteB] = await createClonedDatabases(
      remoteURLBase,
      localDir,
      serialId,
   ...
   */
  it.skip('resolves case 8 - Conflict. Accept ours (delete)', async () => {
   ...
   *  jsonA2:  1 - Accept theirs (create)
   */
  it.skip('resolves case 11 - Conflict. Accept ours (update), case 1 - Accept theirs (create), ', async () => {
    const [dbA, remoteA] = await createDatabase(remoteURLBase, localDir, serialId, {
      conflict_resolve_strategy: 'ours',
    });
    // A puts and pushes
   ...
   */
  it.skip('resolves case 16 - Conflict. Accept ours (update)', async () => {
    const [dbA, remoteA] = await createDatabase(remoteURLBase, localDir, serialId, {
      conflict_resolve_strategy: 'ours',
    });
    // A puts and pushes
   ...
   */
  it.skip('resolves case 17 - Conflict. Accept ours (update)', async () => {
    const [dbA, remoteA] = await createDatabase(remoteURLBase, localDir, serialId, {
      conflict_resolve_strategy: 'ours',
    });
    // A puts and pushes
