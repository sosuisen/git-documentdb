  ...
  describe('merge per document', () => {
    ...
    it('resolves case 17 - Conflict. Accept ours (update)', async () => {
      ...
      expect(syncResult1.commits).toMatchObject({
        local: getCommitInfo([
          putResultA1dash,
          `resolve: 1${dbA.fileExt}(update,${putResultA1dash.file_sha.substr(
            0,
            7
          )},theirs)`,
        ]),
      ...
      expect(syncResult1.commits).toMatchObject({
        local: getCommitInfo([
          ...
          `resolve: 1${dbA.fileExt}(delete,${deleteResultB1.file_sha.substr(
            0,
            7
          )},ours), 2${dbA.fileExt}(update,${putResultB2.file_sha.substr(0, 7)},ours), 3${
            dbA.fileExt
