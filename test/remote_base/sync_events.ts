  ...
  describe('<remote/sync> [event]', () => {
    ...
    describe('change', () => {
      ...
      it('occurs with every retry', async () => {
        ...
        stubPush.onFirstCall().rejects(new RemoteErr.UnfetchedCommitExistsError(''));
    ...
    it('error', async () => {
      ...
      await expect(syncA.trySync()).rejects.toThrowError(RemoteErr.HTTPError404NotFound);
